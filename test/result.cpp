        ...
        {
            {
                auto result1 = Result<std::string, std::string>(
                    Ok(std::string(
                        "Hello there. This is a long string to "
...
TEST_CASE("Result combinators and adapters", "[result]") {
    SECTION("map") {
        {
            ...
            REQUIRE(result2.clone().map([](const auto& x) {
                return x * 2.0;
            }) == Ok(5.0));
        ...
        }
        {
            ...
            REQUIRE(result1.map([](const auto&) {
                return 5;
            }) == Err(std::string("test")));
        }
    }
    SECTION("map_err") {
        {
            ...
            REQUIRE(result1.map_err([](auto) {
                return std::string("cat");
            }) == Err(std::string("cat")));
            REQUIRE(result2.map_err([](auto) {
                return std::string("cat");
            }) == Ok(5));
        }
    }
    SECTION("and") {
        {
            REQUIRE(
                Result<int, int>(Ok(5)).and_(Result<double, int>(Ok(2.5)))
                == Ok(2.5)
            );
            REQUIRE(
                Result<int, int>(Err(5)).and_(Result<double, int>(Ok(2.5)))
                == Err(5)
            );
        }
    }
    ...
    SECTION("or") {
        {
            REQUIRE(
                Result<int, int>(Ok(5)).or_(Result<int, int>(Ok(2))) == Ok(5)
            );
            REQUIRE(
                Result<int, int>(Err(5)).or_(Result<int, double>(Ok(3)))
                == Ok(3)
            );
            REQUIRE(
                Result<int, int>(Err(5)).or_(Result<int, double>(Err(10.0)))
                == Err(10.0)
            );
        }
    }
...
TEST_CASE("Hash", "[result]") {
    ...
    REQUIRE(
        std::hash<Result<int, std::string>>()(result)
        == std::hash<int>()(result.unwrap())
    );
    REQUIRE(
        std::hash<Result<int, std::string>>()(result2)
