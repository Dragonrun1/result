...
public:
    ...
    explicit constexpr Err(const T& val)
        : m_value(val) { }

    explicit constexpr Err(T&& val)
...
public:
    ...
    explicit constexpr Ok(const T& val)
        : m_value(val) { }

    explicit constexpr Ok(T&& val)
        : m_value(std::move(val)) { }
    ...
    template <typename E> explicit constexpr operator Result<T, E>() const& {
        return Result<T, E>(Ok(m_value));
    }

    template <typename E> explicit constexpr operator Result<T, E>() && {
...
namespace details {
    ...
    template <typename T, typename E> class ResultStorage {
    ...
    public:
        ...
        constexpr ResultStorage& operator=(const ResultStorage& rhs
        ...
        constexpr ResultStorage& operator=(ResultStorage&& rhs
        ) noexcept(std::is_nothrow_move_assignable_v<T> && std::is_nothrow_move_assignable_v<E>) {
            destroy();
            m_tag = rhs.m_tag;
...
public:
    ...
    static_assert(
        std::is_same_v<std::remove_reference_t<E>, E>,
        "Result<T, E> cannot store reference types."
        "Try using `std::reference_wrapper`"
    );
    ...
    explicit constexpr Result(Ok<T> value)
        : m_storage(std::move(value)) { }

    explicit constexpr Result(Err<E> value)
        : m_storage(std::move(value)) { }
    ...
    constexpr Result(
        const Result& other
    ) noexcept(std::is_nothrow_copy_constructible_v<details::
                                                        ResultStorage<T, E>>)
        = default;

    constexpr Result& operator=(const Result& other
    ) noexcept(std::is_nothrow_copy_assignable_v<details::ResultStorage<T, E>>)
        = default;

    constexpr Result(
        Result&& other
    ) noexcept(std::is_nothrow_move_constructible_v<details::
                                                        ResultStorage<T, E>>)
        = default;

    constexpr Result& operator=(Result&& other
    ) noexcept(std::is_nothrow_move_assignable_v<details::ResultStorage<T, E>>)
        = default;
    ...
    template <typename T2> Result<T2, E> and_(Result<T2, E> other) {
        if (is_ok()) {
            return other;
        }
        return Result<T2, E>(Err(std::move(*this).err_unchecked()));
    ...
    template <
        ...
        std::enable_if_t<std::is_invocable_r_v<Result<T2, E>, F, T>, int> = 0>
    ...
    template <typename E2> Result<T, E2> or_(Result<T, E2> other) {
        if (is_err()) {
            return other;
        }
        return Result<T, E2>(Ok(std::move(*this).ok_unchecked()));
...
template <typename T> std::ostream& operator<<(std::ostream& stream, unit_t) {
    stream << "()";
    return stream;
}
...
namespace std {
template <typename T, typename E>
struct hash<result::Result<T, E>> { // NOLINT(*-dcl58-cpp)
